  ...
  return (
    <ul className="grid grid-cols-4 gap-5 flex-1 h-fit">
      {students.map((student) => (
        <li
          className="flex flex-col items-center gap-3 h-fit px-5 py-3 bg-neutral-600 rounded-lg shadow-xl cursor-pointer transition ease-in-out duration-300 hover:scale-110 hover:drop-shadow-xl"
          key={student.name}
        >
          {student.photo ? (
